...
forvalues i=1/$N {
	...
	qui: reg var_salidas ln_dist_mil_fac_f1_impy2_placebo ln_dist_mil_fac_f1_placebo f1_impy2 fase_1 i.semana i.code [pw=$W]
...
use "$path\movilidad_final.dta", clear
foreach strep_var in $strep_vars {
	acreg var_salidas `strep_var'_f1_impy1 `strep_var'_f1 f1_impy1 fase_1 i.semana i.code [pw=$W], $se 
	eststo
}
esttab using TableA3.tex, b(%9.4f) se(%9.4f) drop(*_f1 f1_impy1 fase_1 *.semana *.code) se rename(shVictims_70_10_f1_impy1 strep_f1_impy1 DVictims_f1_impy1 strep_f1_impy1 DCentroDetencion_f1_impy1 strep_f1_impy1 ln_centro_det_f1_impy1 strep_f1_impy1 ln_dist_mil_fac_f1_impy1 strep_f1_impy1) coeflabels(strep_f1_impy1 "State repression $\times$ County Imp. Years (1973-1976) $\times$ Lockdown") replace
eststo clear 
...
eststo
use "$path\movilidad_final.dta", clear
acreg var_salidas Dregimientos_f1_impy2 Dregimientos_f1 f1_impy1 fase_1 i.semana i.code [pw=$W], $se 
eststo
esttab using TableA4.tex, b(%9.4f) se(%9.4f) keep(Dregimientos_impy2 Dregimientos_f1_impy2) se coeflabels(Dregimientos_impy2 "Indicator mil. presence $\times$ Imp. Years (1973-1976)" Dregimientos_f1_impy2 "Indicator mil. presence $\times$ County Imp. Years (1973-1976) $\times$ Lockdown") replace
eststo clear 
...
forval i=25(25)150 {
	foreach strep_var in $strep_vars {
		acreg var_salidas `strep_var'_f1_impy2 `strep_var'_f1 f1_impy2 fase_1 i.semana i.code [pw=$W],  spatial latitude(latitud) longitude(longitud) distcutoff(`i')
		eststo
	}
	esttab using TableA10.tex, b(%9.4f) se(%9.4f) drop(*_f1 f1_impy2 fase_1 *.semana *.code) se rename(shVictims_70_10_f1_impy2 strep_f1_impy2 DVictims_f1_impy2 strep_f1_impy2 DCentroDetencion_f1_impy2 strep_f1_impy2 ln_centro_det_f1_impy2 strep_f1_impy2 ln_dist_mil_fac_f1_impy2 strep_f1_impy2) coeflabels(strep_f1_impy2 "State repression $\times$ County Imp. Years (1973-1976) $\times$ Lockdown") append
	eststo clear
}
